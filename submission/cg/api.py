...
@dataclass
class PlayerState:
    active: List[Optional[Pokemon]]  # Active Pokémon (None if the card is facedown). The array size is either 0 or 1.
    ...
    prize: List[Optional[Card]]  # Prize cards (None if the card is facedown). The first element is the bottom of the prize, and the last element is the top.
    handCount: int  # Number of Cards in Hand.
    hand: Optional[List[Card]]  # Hand Card Array. None for the opponent.
...
class State:
    ...
    looking: List[Optional[Card]]  # Looking cards (None if the card is facedown). None if not looking cards.
...
class Option:
    ...
    energyIndex: Optional[int] = None
    count: Optional[int] = None
    inPlayArea: Optional[AreaType] = None
    inPlayIndex: Optional[int] = None
...
class SelectData:
    ...
    deck: Optional[List[Card]]  # An array of cards; None unless selecting cards from the deck.
    contextCard: Optional[Card]  # Which card is the selection concerning? This is sent when the context is "Activate"; otherwise, it is null.
    effect: Optional[Card]  # The card that is activating the effect currently being processed.
    
@dataclass
class Log:
    type: LogType  # Use this parameter to determine which log it is.
    playerIndex: Optional[int] = None
    hasBasicPokemon: Optional[bool] = None
    cardId: Optional[int] = None
    serial: Optional[int] = None
    fromArea: Optional[AreaType] = None
    toArea: Optional[AreaType] = None
    cardIdActive: Optional[int] = None
    serialActive: Optional[int] = None
    cardIdBench: Optional[int] = None
    serialBench: Optional[int] = None
    cardIdBefore: Optional[int] = None
    serialBefore: Optional[int] = None
    cardIdAfter: Optional[int] = None
    serialAfter: Optional[int] = None
    cardIdTarget: Optional[int] = None
    serialTarget: Optional[int] = None
    attackId: Optional[int] = None
    value: Optional[int] = None
    putDamageCounter: Optional[bool] = None
    isRecover: Optional[bool] = None
...
@dataclass
class Observation:
    select: Optional[SelectData]  # Selection information. At the time of the initial deck selection, it will be None.
    logs: list[Log]  # Events that have occurred since the last selection.
    current: Optional[State]  # Current state. At the time of the initial deck selection, it will be None.
    search_begin_input: Optional[str] = None # Input to the search_begin function.
...
@dataclass
class ApiResult:
    state: Optional[SearchState] # Search state.
...
class CardData:
    ...
    hp: int  # Pokémon HP.
    weakness: Optional[EnergyType]  # Pokémon weakness.
    resistance: Optional[EnergyType]  # Pokémon resistance.
    ...
    evolvesFrom: Optional[str]  # If the Pokémon has evolved, then the name of its pre-evolution. Otherwise, None.
